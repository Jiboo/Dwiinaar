...
public class ReusableBufferedInputStream extends FilterInputStream {
    ...
    private int fillbuf(InputStream localIn, byte[] localBuf)
        ...
        if (markpos == 0 && marklimit > localBuf.length) {
            throw new IllegalStateException("Buffer size was too small to fill");
        } else if (markpos > 0) {
    ...
    public synchronized int read(@NonNull byte[] buffer, int byteOffset, int byteCount) throws IOException {
        ...
        if ((byteOffset | byteCount) < 0 || byteOffset > buffer.length || buffer.length - byteOffset < byteCount) {
            throw new ArrayIndexOutOfBoundsException("length: " + buffer.length + ", offset: " + byteOffset + " to " + (byteOffset + byteCount));
        }
        if (byteCount == 0) {
            return 0;
        }
